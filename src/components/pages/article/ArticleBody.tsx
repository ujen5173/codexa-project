...
import CommentsModal from "~/components/models/Comment";
    ...
    <main className="bg-white pb-12 dark:bg-primary">
      <div className="mx-auto max-w-[1000px]">
        ...
        <section className={`relative pt-8 md:pb-0 md:pt-14`}>
          ...
          {tagsLoading ? (
            <div className="mx-auto my-10 flex w-11/12 flex-wrap items-center justify-center gap-2 lg:w-8/12">
              {[1, 2, 3, 4].map((_, index) => (
                <div
                  key={index}
                  className="loading h-4 w-16 rounded-md bg-border-light dark:bg-border"
                ></div>
              ))}
            </div>
          ) : (
            <ArticleTags tags={tagsData!} />
          )}
          ...
          {article.seriesId && <SeriesSection slug={article.slug} />}
        </section>
      </div>
    </main>
  );
